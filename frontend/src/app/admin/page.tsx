...
import { casesApi, profilesApi, authApi, matchesApi, ApiError, type ApiCase, type AppRole } from '../../lib/api';
...
/* ── Rejection reasons ──────────────────────────────────────────
   Shown to the admin as quick choices and surfaced to the reporter on
   the public tracking page, so keep the wording clear and respectful. */
const REJECTION_REASONS = [
  'Insufficient information to verify this report',
  'Duplicate of an existing report',
  'Missing or invalid supporting documents / proof',
  'Suspected false, prank, or spam report',
  'Report does not fall within our coverage area',
] as const;
const REJECTION_OTHER = 'Other (specify a reason)';
...
export default function AdminPage() {
  ...
  const notifHighlight              = useRef<Set<string>>(new Set());
  ...
  const [rejectTarget, setRejectTarget] = useState<PendingCase | null>(null);
  ...
  /* ── Notifications — derived from live admin state ───────── */
  interface AdminNotif {
    id: string;
    title: string;
    detail: string;
    time?: string;        // ISO timestamp, when available
    view: View;
    accent: string;
    icon: React.ReactNode;
  }

  const notifications: AdminNotif[] = useMemo(() => {
    const list: AdminNotif[] = [];

    /* New public submissions awaiting approval */
    queue.forEach(c => {
      list.push({
        id:     `pending-${c.id}`,
        title:  'New report awaiting approval',
        detail: `${c.name} · ${c.type === 'missing' ? 'Missing person' : 'Unidentified person'}${c.barangay ? ` · ${c.barangay}` : ''}`,
        time:   c.submittedAt,
        view:   'queue',
        accent: '#d35400',
        icon:   <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
      });
    });

    /* Possible case matches surfaced by the matching engine */
    matches.forEach(m => {
      list.push({
        id:     `match-${m.id}`,
        title:  m.flagged ? 'Flagged match needs review' : 'Possible case match found',
        detail: `${m.missing.name} ↔ ${m.unidentified.name} · ${m.score}% confidence`,
        view:   'matches',
        accent: m.flagged ? '#c0392b' : '#27ae60',
        icon:   <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M13 6h3a2 2 0 0 1 2 2v7"/><line x1="6" y1="9" x2="6" y2="21"/></svg>,
      });
    });

    /* Newest first — items with timestamps sort above those without */
  ...
  /* Load previously-seen notification ids from this browser */
  useEffect(() => {
    try {
      const raw = localStorage.getItem('tuklas-admin-seen-notifs');
      if (raw) setSeenNotifIds(new Set(JSON.parse(raw) as string[]));
    } catch { /* ignore unavailable/invalid storage */ }
  }, []);

  /* Open/close the dropdown; opening clears the unread badge */
  function toggleNotif() {
    setNotifOpen(open => {
      const next = !open;
      if (next) {
        /* Snapshot what was unread so the panel can highlight it this session */
        notifHighlight.current = new Set(
          notifications.filter(n => !seenNotifIds.has(n.id)).map(n => n.id),
        );
        /* Mark everything currently visible as seen + persist */
        setSeenNotifIds(prev => {
          const merged = new Set(prev);
          notifications.forEach(n => merged.add(n.id));
          try {
            localStorage.setItem('tuklas-admin-seen-notifs', JSON.stringify([...merged]));
          } catch { /* ignore */ }
          return merged;
        });
      }
  ...
  function openNotifItem(n: AdminNotif) {
    setView(n.view);
    setNotifOpen(false);
  }
  ...
  /* Reject — open the rejection-reason modal first */
  function handleReject(id: string) {
    const target = queue.find(c => c.id === id) ?? null;
    setRejectReason('');
    setRejectOtherText('');
    setRejectTarget(target);
  }

  /* Confirmed reject — record the reason so the reporter can see it when
     tracking, then remove from the local queue only on success */
  function confirmReject(id: string, reason: string) {
    setRejectTarget(null);
    setRejectBusy(true);
    casesApi.reject(id, reason)
      .then(() => {
        setQueue(q => q.filter(c => c.id !== id));
        showToast('Case rejected. The reporter can now see the reason when tracking.', 'success');
      })
      .catch(err => {
        /* If the case was already gone, drop it from the queue so the UI
           matches the server instead of leaving a phantom row behind. */
        if (err instanceof ApiError && err.status === 404) {
          setQueue(q => q.filter(c => c.id !== id));
          showToast('Case was already removed.', 'info');
        } else {
          showToast(`Rejection failed: ${err.message}`, 'info');
        }
      })
  ...
  return (
    <div className="admin-shell">
      ...
      {/* Reject modal — pick a reason that the reporter will see on tracking */}
      {rejectTarget && (() => {
        const isOther     = rejectReason === REJECTION_OTHER;
        const finalReason = isOther ? rejectOtherText.trim() : rejectReason;
        ...
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`Reject case ${rejectTarget.name}`}
          onClick={e => { if (e.target === e.currentTarget) setRejectTarget(null); }}
        >
          <div className="mgmt-modal" style={{ maxWidth: 480 }}>
            <div className="mgmt-modal-header">
              <h2 className="mgmt-modal-title">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16" aria-hidden="true">
              ...
              <button className="mgmt-modal-close" onClick={() => setRejectTarget(null)} aria-label="Close">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" width="16" height="16" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
              </button>
            </div>
            <div className="mgmt-modal-body">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.65rem 0.75rem', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e8eaed', marginBottom: '0.85rem' }}>
                <span className={`vtable-type-pill ${rejectTarget.type}`}>{rejectTarget.type}</span>
                <div>
                  <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{rejectTarget.name}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--color-text-light)' }}>
                    {rejectTarget.reference || '—'}{rejectTarget.barangay ? ` · ${rejectTarget.barangay}` : ''}
                  </div>
                </div>
              </div>

              <p className="mgmt-form-label" style={{ marginBottom: '0.5rem' }}>
                Reason for rejection <span style={{ color: '#701515' }}>*</span>
              </p>
              <div role="radiogroup" aria-label="Reason for rejection" style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {[...REJECTION_REASONS, REJECTION_OTHER].map(reason => {
                  const selected = rejectReason === reason;
                  return (
                    <label
                      key={reason}
                      className={`reject-reason-option${selected ? ' selected' : ''}`}
                    >
                      <input
                        type="radio"
                        name="reject-reason"
              ...
              {isOther && (
                <textarea
                  ...
                  placeholder="Describe the reason for rejecting this report…"
                  rows={3}
                  maxLength={1000}
                  autoFocus
                  style={{ resize: 'vertical', lineHeight: 1.5, marginTop: '0.6rem', width: '100%' }}
                  aria-label="Custom rejection reason"
                />
              )}

              <p style={{ fontSize: '0.78rem', lineHeight: 1.55, margin: '0.85rem 0 0', color: 'var(--color-text-light)' }}>
                The report will be hidden from the public, and the reporter will see this reason when they track their reference number.
              </p>
            </div>
            <div className="mgmt-modal-footer">
              <button type="button" className="mgmt-btn-ghost" onClick={() => setRejectTarget(null)} disabled={rejectBusy}>Cancel</button>
              <button
                type="button"
                className="vtable-btn reject"
                style={{ padding: '0.5rem 1rem', opacity: canReject ? 1 : 0.5 }}
                onClick={() => confirmReject(rejectTarget.id, finalReason)}
                disabled={rejectBusy || !canReject}
              >
                {rejectBusy ? 'Rejecting…' : 'Reject Report'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="admin-topbar-actions">
            ...
            <div className="admin-notif" ref={notifRef}>
              <button
                ...
                aria-label={unreadCount > 0 ? `Live alerts, ${unreadCount} unread` : 'Live alerts'}
                aria-haspopup="true"
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="admin-notif-badge" aria-hidden="true">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <button
                          key={n.id}
                          type="button"
                          className={`admin-notif-item${notifHighlight.current.has(n.id) ? ' unread' : ''}`}
                          onClick={() => openNotifItem(n)}
                        >
                          <span className="admin-notif-icon" style={{ background: `${n.accent}18`, color: n.accent }}>
                            {n.icon}
                          </span>
                          <span className="admin-notif-text">
                            <span className="admin-notif-title">{n.title}</span>
                            <span className="admin-notif-detail">{n.detail}</span>
                            {n.time && <span className="admin-notif-time">{timeAgo(n.time)}</span>}
                          </span>
                          {notifHighlight.current.has(n.id) && <span className="admin-notif-unread-dot" aria-label="Unread" />}
                        </button>
                      ))
                    )}
                  </div>
